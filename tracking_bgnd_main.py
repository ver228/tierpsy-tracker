...
for ind_frame in range(5):
    ...
    if nChunk_prev != nChunk:
        ...
        mask_bgnd = cv2.adaptiveThreshold(bgnd[bgndInd,:,:],1,cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV,61,35)    
    nChunk_prev = nChunk;
    ...
    #solidity = np.array([x.solidity for x in props]); #solidity is quite slow to calculate, unless I have a good reason to do it, I do not see the point
...
#       
    ...
    results_list = zip(*(indexList, frames, coord[:,0], coord[:,1], area, perimeter, 
