...
if __name__ == "__main__":
    '''
    Select valid trajectories using area/length, width/length and length
    ...
    skeletons_file = '/Users/ajaver/Tmp/Results/Locomotion_videos_for_analysis_2015/142 1_skeletons.hdf5'
    with pd.HDFStore(skeletons_file, 'r') as ske_file_id:
        trajectories_data = ske_file_id['/trajectories_data']
        trajectories_data = trajectories_data[['frame_number', 'worm_index_joined', 'has_skeleton']]

    with tables.File(skeletons_file, 'r') as ske_file_id:
        trajectories_data['area'] = ske_file_id.get_node('/contour_area')[:]
        trajectories_data['length'] = ske_file_id.get_node('/skeleton_length')[:]
        mid_point = int(round(ske_file_id.get_node('/contour_width').shape[1]/2))
        
        dd = ske_file_id.get_node('/contour_width')[:, (mid_point-2):(mid_point+3)]
        trajectories_data['width'] = np.mean(dd, axis = 1)
        del dd
        trajectories_data['WL_ratio'] = trajectories_data['width']/trajectories_data['length']
        trajectories_data['WA_ratio'] = trajectories_data['width']/trajectories_data['area']
        trajectories_data['AL_ratio'] = trajectories_data['area']/trajectories_data['length']
   
    trajectories_data = trajectories_data[trajectories_data['has_skeleton']!=0]
#%%z
    #sns.pairplot(trajectories_data, diag_kind="kde", vars = ['area', 'length', 'width', 'WL_ratio', 'AL_ratio', 'WA_ratio']);
    
#%%
    X = trajectories_data[['area', 'length', 'width', 'WL_ratio', 'AL_ratio', 'WA_ratio']]    
    X = (X - X.mean())/X.std()
    #sns.pairplot(X)#, diag_kind="kde")
    #D = preprocessing.StandardScaler(X, with_mean = False)    
    #sns.pairplot(trajectories_data[['area', 'length', 'width']])
    outlier_detector = EllipticEnvelope(contamination=.1, assume_centered=True);
    outlier_detector.fit(X)
    y_pred = outlier_detector.decision_function(X)
    #%%
    X['prediction']  = y_pred<0.35   
    sns.pairplot(X, vars =['area', 'length', 'width'], hue = 'prediction')
