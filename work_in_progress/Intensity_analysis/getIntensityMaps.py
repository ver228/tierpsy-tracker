...
if __name__ == '__main__':
    ...
    with tables.File(masked_image_file, 'r')  as mask_fid, \
        ...
        #create array to save the intensities
        filters = tables.Filters(complevel=5, complib='zlib', shuffle=True)
        worm_index_tab = int_file_id.create_carray("/", "worm_index", \
                                   tables.Int32Atom(dflt=-1), \
                                   (tot_rows,), filters = filters);
        
        frame_number_tab = int_file_id.create_carray("/", "frame_number", \
                                   tables.Int32Atom(dflt=-1), \
                                   (tot_rows,), filters = filters);
        ...
        for frame, frame_data in trajectories_df.groupby('frame_number'):
            ...
            for segworm_id, row_data in frame_data.iterrows():
                ...
                if not np.any(np.isnan(skeleton)): 
                    ...
                    worm_index_tab[segworm_id] = worm_index
                    frame_number_tab[segworm_id] = row_data['frame_number']
