# -*- coding: utf-8 -*-
"""
Created on Fri Jun  5 20:36:09 2015
...
if __name__ == "__main__":    
    ...
    for frame_number in range(initial_frame, last_frame, buffer_size):
        ...
        #load image buffer
        image_buffer = mask_dataset[frame_number:(frame_number+buffer_size),1200:1500,300:600]
        ...
        for ROI_ind, ROI_cnt in enumerate(ROIs[0:10]):
            ...
            for buff_ind in range(buff_tot):
                ...
                ROI_border = cv2.dilate(ROI_valid.astype(np.uint8), np.ones((3,3)))- ROI_valid
                ROI_border_ind = np.where(ROI_border)
                
#                _, ROI_border_ind, _ =  cv2.findContours(ROI_valid.astype(np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)  
#                
...
#                        ('major_axis', float)])
#
#    with pd.HDFStore(trajectories_file, mode = 'r') as feature_fid:
#        feature_table = feature_fid['/plate_worms']
#        feature_table = feature_table[['worm_index', 'frame_number', 'major_axis', ]]
#        
#    #select only indexes larger than min_track_size
#    track_size = feature_table['worm_index'].value_counts()
#    valid_tracks = track_size[track_size>=min_track_size].index
#    feature_table = feature_table[feature_table['worm_index'].isin(valid_tracks)]
#    
#    #get the index of the max and minimum frame for each track
#    track_row_lims = feature_table[['worm_index','frame_number']].groupby('worm_index').aggregate([np.argmin, np.argmax])
#    track_row_lims = track_row_lims['frame_number']
#    
#    last_frames_rows = feature_table.loc[track_row_lims['argmax'].values]
#    first_frames_rows = feature_table.loc[track_row_lims['argmin'].values]
#
#    for last_row in last_frames_rows.iterrows():
#        print(last_row)
#    
...
##                if R[indmin] <= last_frames['major_axis'][kk]:
