...
for i,x in  enumerate(colorpalette):
    colorpalette[i] = tuple([float(y)/255 for y in x])
...
for worm_index in good_index:
    dat = df[df['worm_index']==worm_index][['coord_x', 'coord_y', 'frame_number']]
...
figure_list = {};
    ...
    #figure_list[worm_index]['writer'] = animation.FFMpegWriter(fps=5, extra_args=['-vcodec', 'libx264'])
...
for frame in range(0, df['frame_number'].max()):
    ...
    for worm_index in index2search:
        ...
        if frame == smoothed_CM[worm_index]['first_frame']:
            ...
            figure_list[worm_index]['writer'] = writeVideoffmpeg(movie_save_name, width=w, height=h)
        ...
        worm = wormsInFrame[wormsInFrame['worm_index'] == worm_index]
        ...
        if (len(threshold) == 1) and (segworm_id < 0):
            ...
            worm_img= cv2.cvtColor(worm_img, cv2.COLOR_GRAY2RGB);
            worm_img[:,:,1][worm_mask!=0] = 150
        
        plt.figure(figure_list[worm_index]['fig'].number)
        plt.clf()
        ...
        if (len(segworm_id)==1) and (segworm_id >= 0):
            ...
            plt.plot(xx[0],yy[0], 'o', color='w')
            #plt.plot(xx[-1],yy[-1], 's', color=tableau20[15])
        
        figure_list[worm_index]['fig'].canvas.draw()
        buf = figure_list[worm_index]['fig'].canvas.tostring_argb();
        figure_list[worm_index]['writer'].write(buf)
