...
try:
    ...
    SCRIPT_COMPRESS_WORKER = [
        os.path.join(
            sys._MEIPASS,
            'compressSingleWorker')]
    if os.name == 'nt':
...
class compressLocal:
    ...
    def start(self):
        ...
        if self.final_has_finished == 0 and self.tmp_video_file != self.video_file:
            print_flush(
                self.base_name +
                ' Copying video file to local temporary directory.')
        ...
        # this might be more logically group in main_code, but this operation
        # can and should be do remotely if required
        if self.final_has_finished == 1:
            # The file finished to be processed but the additional data was not
            # stored. We can do this remotely.
        ...
        # parameters to calculate the mask, this will return a list with
        # necessary commands in case I want to use it as a subprocess
    ...
    def create_script(self):
        ...
        # replace bool values by a letter, otherwise one cannot parse them to
        # the command line
    ...
    def get_file_names(self):
        ...
        self.base_name = self.video_file.rpartition(
            '.')[0].rpartition(os.sep)[-1]

        self.masked_image_file = os.path.join(
            self.mask_dir, self.base_name + '.hdf5')
        self.masked_image_file = os.path.abspath(self.masked_image_file)
        assert self.masked_image_file != self.video_file

        self.tmp_mask_file = os.path.join(
            self.tmp_mask_dir, self.base_name + '.hdf5')
        self.tmp_mask_file = os.path.abspath(self.tmp_mask_file)

        if self.is_copy_video:
            self.tmp_video_file = os.path.join(
                self.tmp_mask_dir, os.path.split(
                    self.video_file)[1])
    ...
    def get_start_point(self):
        ...
        try:
            with h5py.File(self.masked_image_file, "r") as mask_fid:
                self.final_has_finished = mask_fid[
                    '/mask'].attrs['has_finished']
        except (OSError, KeyError):
            self.final_has_finished = 0

        if self.final_has_finished == 0:
            # check if a finished temporal mask exists. The processes was
            # interrupted during copying.
            try:
                with h5py.File(self.tmp_mask_file, "r") as mask_fid:
                    MAX_CONTROL_FLAG = 2
                    self.is_calculate_mask = (
                        mask_fid['/mask'].attrs['has_finished'] < MAX_CONTROL_FLAG)
            except (OSError, KeyError):
    ...
    def clean(self):
        if self.final_has_finished == 0:
            print_flush(
                self.base_name +
                ' Moving files to final destination and removing temporary files.')
            if os.path.abspath(
                    self.tmp_video_file) != os.path.abspath(
                    self.video_file):
            ...
            if sys.platform == 'darwin':
                ...
                # Otherwise only the owner would be able to change the
                # ummutable flag.
                os.chmod(self.masked_image_file, stat.S_IRUSR | stat.S_IRGRP |
        ...
        time_str = str(
            datetime.timedelta(
                seconds=round(
                    ...
                    self.start_time)))
        print_flush(
            '%s Finished. Total time = %s' %
            (self.base_name, time_str))
...
class compressLocal_parser(compressLocal):
    ...
    def __init__(self, sys_argv=''):
        ...
        self.parser = argparse.ArgumentParser(
            description="Compress worm videos into masked hdf5 files processing data first into the local drive.")
        self.parser.add_argument('video_file', help='Original video.')
        self.parser.add_argument(
            'mask_dir',
            help='Final directory where the compressed files are going to be stored')
        self.parser.add_argument(
            '--tmp_mask_dir',
            default='',
        ...
        self.parser.add_argument(
            '--json_file',
            default='',
            help='File (.json) containing the compressed parameters.')
        self.parser.add_argument(
            '--is_single_worm',
            action='store_true',
            help='Indicates if the video corresponds to the single worm case.')
        self.parser.add_argument(
            '--is_copy_video',
            action='store_true',
