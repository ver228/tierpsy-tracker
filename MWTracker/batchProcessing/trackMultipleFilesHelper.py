...
class checkTrackFiles(checkVideoFiles):
    ...
    def __init__(
        ...
        if not os.path.exists(mask_dir_root):
            raise FileExistsError(
                'The masks root directory %s does not exist.' %
                mask_dir_root)
        ...
        if json_file:  # empty file is accepted (Default parameters)
            ...
            if not os.path.exists(json_file):
                raise FileExistsError(
                    'The parameters file %s does not exist.' %
                    json_file)
        ...
        self.end_point_N = checkpoint[
            'END'] if not end_point else checkpoint[end_point]
    ...
    def generateIndCMD(self, masked_image_file):
        ...
        if self.tmp_dir_root:
            tmp_mask_dir = getDstDir(
                mask_dir, self.mask_dir_root, os.path.join(
                    self.tmp_dir_root, 'MaskedVideos'))
            tmp_results_dir = getDstDir(
                mask_dir, self.mask_dir_root, os.path.join(
                    self.tmp_dir_root, 'Results'))
            ...
            # if tmp_dir_root is empty just use the same directory (no tmp
            # files)
    ...
    def checkIndFile(self, masked_image_file):
        ...
        if self.is_single_worm:
            _, _, skeletons_file, _, _, _ = constructNames(
                masked_image_file, results_dir)

        # check that the file finished correctly and that there is no
        # force_start_point specified
        if (start_point > self.end_point_N or start_point ==
                checkpoint['END']) and not self.force_start_point:
