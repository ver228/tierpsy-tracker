...
def isBadVideo(video_file, is_single_worm):
    try:
        ...
        if is_single_worm:
            ...
            try:
                # this function will throw and error if the .info.xml or
                # .log.csv are not found
...
def isBadMask(masked_image_file):
    ...
    try:
        with tables.File(masked_image_file, 'r') as mask_fid:
            ...
            if ('has_finished' in mask_node._v_attrs) and (
                    mask_node._v_attrs['has_finished'] < 1):
                raise ValueError
...
class checkVideoFiles:
    ...
    def __init__(self, video_dir_root, mask_dir_root,
        ...
        if not os.path.exists(video_dir_root):
            raise FileExistsError(
                'The root video directory %s does not exist.' %
                video_dir_root)
    ...
    def filterFiles(self, valid_files):
        ...
        for ii, video_file in enumerate(valid_files):
            ...
            if (ii % 10) == 0:
                print('Checking file %i of %i. Total time: %s' %
                      (ii + 1, len(valid_files), progress_timer.getTimeStr()))
    ...
    def generateIndCMD(self, video_file):
        ...
        if self.is_single_worm:
            cmd.append('--is_single_worm')
        if self.is_copy_video:
            cmd.append('--is_copy_video')

        return cmd

    def cleanPrevious(self):
        ''' Delete any bad finished files.'''
        for video_file, masked_image_file in self.filtered_files[
                'FINISHED_BAD']:
