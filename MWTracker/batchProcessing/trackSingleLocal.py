...
class trackLocal:
    ...
    def start(self):
        ...
        self.main_input_params = [[self.tmp_mask_file,
                                   self.tmp_results_dir],
                                  {'json_file': self.json_file,
                                   'start_point': self.analysis_start_point,
                                   'end_point': self.end_point,
                                   'is_single_worm': self.is_single_worm,
                                   'use_skel_filter': self.use_skel_filter,
                                   'use_manual_join': self.use_manual_join,
    ...
    def clean(self):
        ...
        if self.is_single_worm and self.end_point >= checkpoint['SKE_CREATE']:
            # in the case of single worm it can exit before due to problems in
            # the stage aligment
        ...
        print_flush('%s  Finished in %s. Total time %s' %
                    (self.base_name, self.last_check_point, time_str))

    def main_code(self):
        # start the analysis
        getTrajectoriesWorker(
            *self.main_input_params[0],
            **self.main_input_params[1])
    ...
    def assign_tmp_dir(self, tmp_mask_dir, tmp_results_dir):
        if tmp_mask_dir:
            ...
            if not tmp_results_dir:
                ...
                self.tmp_results_dir = self.tmp_mask_dir.replacereplace(
                    'MaskedVideos', 'Results')

        if tmp_results_dir:
            self.tmp_results_dir = tmp_results_dir
            if not tmp_mask_dir:
                # deduce tmp_mask_dir from tmp_results_dir
                self.tmp_mask_dir = self.tmp_results_dir.replacereplace(
                    'Results', 'MaskedVideos')
    ...
    def getStartPoints(self):
        ...
        # starting point calculated from the files in the final destination
        self.final_start_point = getStartingPoint(
            self.masked_image_file, self.results_dir)
        # starting point calculated from the files in the temporal directory
        self.tmp_start_point = getStartingPoint(
            self.tmp_mask_file, self.tmp_results_dir)
        self.analysis_start_point = max(
            self.final_start_point,
            self.tmp_start_point)  # starting point for the analysis

        if self.force_start_point_str:
            self.analysis_start_point = min(
                self.analysis_start_point, checkpoint[
                    self.force_start_point_str])
    ...
    def copyFilesFromFinal(self):
        # check that the program didn't finished and that not forced start
        # point was not specified
        if ((self.final_start_point == checkpoint[
                'END']) or self.final_start_point > self.end_point) and not self.force_start_point_str:
        ...
        # find what files we need to copy from the final destination if the
        # analysis is going to resume from a later point
        files2copy = []

        if self.analysis_start_point <= checkpoint['INT_PROFILE']:
            # we do not need the mask to calculate the features
            try:
                # check if there is already a finished/readable temporary mask
                # file in current directory otherwise copy the
                ...
                with h5py.File(self.masked_image_file, "r") as mask_fid:
                    # check if the video to mask conversion did indeed finished
                    # correctly
        ...
        if self.tmp_start_point < self.final_start_point:
            ...
            # and final_start_point <= checkpoint['SKE_CREATE']:
            if self.final_start_point > checkpoint['TRAJ_CREATE']:
    ...
    def copyFilesToFinal(self):
        ...
        def wasProccesed(str_point):
            return checkpoint[str_point] >= self.final_start_point and checkpoint[
                str_point] <= self.end_point
        ...
        # in the case of single worm if the stage aligment fails the analysis
        # will be interruped before
        if self.last_check_point != 'STAGE_ALIGMENT':
            if wasProccesed('INT_PROFILE'):
                files2copy += [(self.intensities_tmp, self.results_dir)]

            if wasProccesed(
                    'FEAT_CREATE') and self.last_check_point != 'CONTOUR_ORIENT':
    ...
    def cleanTmpFiles(self):
        ...
        if os.path.abspath(
                self.tmp_mask_file) != os.path.abspath(
                self.masked_image_file):
            if os.path.exists(self.tmp_mask_file):
                os.remove(self.tmp_mask_file)

        # this files must exists at this point in the program. Let's check it
        ...
        if self.last_check_point != 'STAGE_ALIGMENT':
            ...
            if self.end_point >= checkpoint[
                    'FEAT_CREATE'] and self.last_check_point != 'CONTOUR_ORIENT':
                assert os.path.exists(self.features_file)

            if self.end_point >= checkpoint[
                    'FEAT_MANUAL_CREATE'] and self.use_manual_join:
                assert os.path.exists(self.feat_ind_file)

        # delete the results temporary files
        if os.path.abspath(
                self.tmp_results_dir) != os.path.abspath(
                self.results_dir):
            if os.path.exists(self.trajectories_tmp):
                os.remove(self.trajectories_tmp)
            if os.path.exists(self.skeletons_tmp):
                os.remove(self.skeletons_tmp)
            if os.path.exists(self.intensities_tmp):
                os.remove(self.intensities_tmp)
            if os.path.exists(self.features_tmp):
                os.remove(self.features_tmp)
            if os.path.exists(self.feat_ind_tmp):
                os.remove(self.feat_ind_tmp)
...
class trackLocal_parser(trackLocal):
    ...
    def __init__(self, sys_argv=''):
        ...
        self.parser = argparse.ArgumentParser(
            description="Track the worm's hdf5 files in the local drive.")
        self.parser.add_argument(
            'masked_image_file',
            help='Fullpath of the .hdf5 with the masked worm videos')
        self.parser.add_argument(
            'results_dir',
            help='Final directory where the tracking results are going to be stored')
        self.parser.add_argument(
            '--tmp_mask_dir',
            default='',
        ...
        super(
            ...
            self).__init__(
            **vars(args),
            cmd_original=subprocess.list2cmdline(sys_argv))
