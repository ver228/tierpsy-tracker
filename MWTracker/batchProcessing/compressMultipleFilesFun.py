...
compress_parser = argparse.ArgumentParser(description="Compress video files in the local drive using several parallel processes")

compress_parser.add_argument('video_dir_root', help='Root directory with the raw videos.')
compress_parser.add_argument('mask_dir_root', help='Root directory where the masked hdf5 files are going to be saved.')
...
compress_parser.add_argument('--tmp_dir_root', default=compress_dflt_vals['tmp_dir_root'], \
	help='Temporary directory where files are going to be stored')

compress_parser.add_argument('--pattern_include', default = compress_dflt_vals['pattern_include'], help = 'Pattern used to find the valid video files in video_dir_root')
compress_parser.add_argument('--pattern_exclude', default = compress_dflt_vals['pattern_exclude'], help = 'Pattern used to exclude files in video_dir_root')
compress_parser.add_argument('--max_num_process', default=compress_dflt_vals['max_num_process'], type=int, help='Max number of process to be executed in parallel.')
compress_parser.add_argument('--refresh_time', default=compress_dflt_vals['refresh_time'], type=float, help='Refresh time in seconds of the process screen.')

compress_parser.add_argument('--is_single_worm', action='store_true', help = 'This flag indicates if the video corresponds to the single worm case.')
compress_parser.add_argument('--only_summary', action='store_true', help='Use this flag if you only want to print a summary of the files in the directory.')
compress_parser.add_argument('--clean_previous', action='store_true', help='Use this flag if you want to delete invalid proccesed files from previous analysis.')
