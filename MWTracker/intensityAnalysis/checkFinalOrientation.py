...
def getHeadProvInt(intensities_file, trajectories_worm, min_block_size, peak_search_limits):
    ...
    #search for the peaks in the intensity profile (the head typically have a minimum, follow by a maximum, then a minimum and then a maxima)    
    peaks_ind = searchIntPeaks(median_int, peak_search_limits = peak_search_limits)
