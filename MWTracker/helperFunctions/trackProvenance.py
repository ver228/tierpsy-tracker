...
def execThisPoint(thisPoint, func, argkws, output_file, commit_hash, cmd_original):
    ...
    variables2save = {'func_name':func.__name__, 'func_arguments':json.dumps(argkws), 
    'commit_hash':commit_hash, 'cmd_original':cmd_original}
    ...
    with tables.File(output_file, 'r+') as fid:
        ...
        if not '/' + main_node in fid: fid.create_group('/', main_node)
        if '/'.join(['', main_node, thisPoint])  in fid: fid.remove_node('/' + main_node, thisPoint)
