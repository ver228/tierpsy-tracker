...
# the order of the list is very IMPORTANT, and reflects the order where is
# step is done
checkpoint_label = [
    'TRAJ_CREATE',
    'TRAJ_JOIN',
...
def getStartingPoint(masked_image_file, results_dir):
    ...
    # at this point the skeleton file must exists so we only check for the
    # correct level in the skeleton file
    with tables.File(skeletons_file, "r") as ske_file_id:
        skeleton_table = ske_file_id.get_node('/skeleton')
    ...
    try:
        ...
        # if there is any problem while reading the file, create it again
        with tables.File(skeletons_file, 'r') as ske_file_id:
            if 'worm_label' in ske_file_id.get_node(
                    '/trajectories_data').colnames:
                return checkpoint['FEAT_MANUAL_CREATE']
...
def constructNames(masked_image_file, results_dir):
    ...
    ext2add = [
        ...
        'intensities']
    for ext in ext2add:
        output += [os.path.abspath(os.path.join(results_dir,
                                                base_name + '_' + ext + '.hdf5'))]
...
class getTrajectoriesWorker():
    ...
    def __init__(
        ...
        # get repository commit hash numbers (useful to determine what version
        # of the code was executed)
        ...
        if self.start_point < 0:
            self.start_point = getStartingPoint(
        ...
        if self.start_point <= checkpoint['SKE_CREATE']:
        ...
        # derive the inputs, functions, and output requires for each point in
        # the analysis
            ...
            if self.is_single_worm:
                if current_point == 'INT_PROFILE':
                    ...
                    if isBadStageAligment(self.skeletons_file):
                        # break, bad video we do not need to calculate anything
                        # else.
                ...
                if current_point == 'FEAT_CREATE':
                    if not hasExpCntInfo(self.skeletons_file):
                        ...
                        execThisPoint(
                            'CONTOUR_ORIENT',
                            **self.points_parameters['CONTOUR_ORIENT'],
                            commit_hash=self.commit_hash,
                            cmd_original=self.cmd_original)
        ...
        time_str = str(
            datetime.timedelta(
                seconds=round(
                    ...
                    initial_time)))
        print_flush(
            '%s  Finished in %s. Total time %s' %
            (self.base_name, current_point, time_str))


track_worker_parser = argparse.ArgumentParser(
    description="Track woms in an individual video hdf5 file.")
track_worker_parser.add_argument('masked_image_file', help='hdf5 video file.')
track_worker_parser.add_argument(
    'results_dir',
