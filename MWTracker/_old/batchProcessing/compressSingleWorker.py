...
def compressSingleWorker(
    ...
    try:
        # try to get the has_finished flag, if this fails the file is likely to
        # be corrupted so we can start over again
        with h5py.File(masked_image_file, "r") as mask_fid:
            has_finished = mask_fid['/mask'].attrs['has_finished']

            if is_single_worm and not all(
                dd in mask_fid for dd in [
                    'stage_log',
                    'xml_info']):
    ...
    if has_finished:
        ...
        # This function will throw an exception if the additional files do not
        # exists, before attempting to calculate anything
        if is_single_worm:
