...
class BatchProcessing_GUI(QMainWindow):
	def __init__(self):
		...
		self.ui.checkBox_isCompress.stateChanged.connect(self.enableCompressInput)
		...
		self.ui.lineEdit_patternInComp.setText(compress_dflt_vals['pattern_include'])
		self.ui.lineEdit_patternExcComp.setText(compress_dflt_vals['pattern_exclude'])
		self.ui.lineEdit_patternInTrack.setText(track_dflt_vals['pattern_include'])
		self.ui.lineEdit_patternExcTrack.setText(track_dflt_vals['pattern_exclude'])

		assert compress_dflt_vals['max_num_process'] == track_dflt_vals['max_num_process']
		assert compress_dflt_vals['tmp_dir_root'] == track_dflt_vals['tmp_dir_root']

		lineEditDragDrop(self.ui.lineEdit_txtFileList, self.updateTxtFileList, os.path.isfile)
		lineEditDragDrop(self.ui.lineEdit_paramFile, self.updateParamFile, os.path.isfile)
		lineEditDragDrop(self.ui.lineEdit_videosDir, self.updateVideosDir, os.path.isdir)
		lineEditDragDrop(self.ui.lineEdit_masksDir, self.updateMasksDir, os.path.isdir)
		lineEditDragDrop(self.ui.lineEdit_resultsDir, self.updateResultsDir, os.path.isdir)
		lineEditDragDrop(self.ui.lineEdit_tmpDir, self.updateTmpDir, os.path.isdir)
	...
	def getTxtFileList(self):
		videos_list, _ = QFileDialog.getOpenFileName(self, "Select a text file with a list of files to be analyzed.", 
	...
	def getTmpDir(self):
		tmp_dir_root = QFileDialog.getExistingDirectory(self, "Selects the directory where the hdf5 masked videos will be stored", 
		self.tmp_dir_root)
		if tmp_dir_root:
			self.updateTmpDir(tmp_dir_root)
	...
	def getVideosDir(self):
		videos_dir = QFileDialog.getExistingDirectory(self, "Selects the directory where the original video files are stored.", 
		self.videos_dir)
		if videos_dir:
	...
	def getResultsDir(self):
		results_dir = QFileDialog.getExistingDirectory(self, 
			"Selects the directory where the analysis results will be stored", self.results_dir)
		if results_dir:
			self.updateResultsDir(results_dir)
	...
	def getMasksDir(self):
		mask_files_dir = QFileDialog.getExistingDirectory(self, 
			"Selects the directory where the hdf5 masked videos will be stored", self.mask_files_dir)
		if mask_files_dir:
			self.updateMasksDir(mask_files_dir)
	...
	def getParamFile(self):
		param_file, _ = QFileDialog.getOpenFileName(self, "Find parameters file", 
		'', "JSON files (*.json);; All (*)")
		if param_file:
	...
	def updateParamFile(self, param_file):
		...
		if param_file:
			try:
				...
				QMessageBox.critical(self, 'Cannot read parameters file.', 
					"Cannot read parameters file. Try another file", QMessageBox.Ok)
				return
	...
	def startAnalysis(self):
		...
		if not is_compress and not is_track:
			QMessageBox.critical(self, 'Error', 
					"Neither compression or selection is selected. No analysis to be done.", QMessageBox.Ok)
		...
		if is_compress:
			...
			if not os.path.exists(compress_args['video_dir_root']):
				QMessageBox.critical(self, 'Error', 
					"The videos directory does not exist. Please select a valid directory.", QMessageBox.Ok)
				return
		
		track_args = {}
		if is_track:
			track_args = self.readTrackArgs()
			if not is_compress and not os.path.exists(track_args['mask_dir_root']):
				QMessageBox.critical(self, 'Error', 
					"The masks directory does not exist. Please select a valid directory.", QMessageBox.Ok)
				return
		...
		analysis_worker = WorkerFunQt(analysis_fun, {'compress_args':compress_args, 'track_args':track_args})
	...
	def readCompressArgs(self):
		...
		compress_vals['is_single_worm'] = self.ui.checkBox_isSingleWorm.isChecked()
		compress_vals['json_file'] = self.ui.lineEdit_paramFile.text()
		compress_vals['is_copy_video'] = self.ui.checkBox_isCopyVideo.isChecked()
		if self.ui.checkBox_txtFileList.isChecked():
			compress_vals['videos_list'] = self.ui.lineEdit_txtFileList.value()
		else:
			compress_vals['pattern_include'] = self.ui.lineEdit_patternInComp.text()
			compress_vals['pattern_exclude'] = self.ui.lineEdit_patternExcComp.text()
	...
	def readTrackArgs(self):
		...
		if self.ui.checkBox_txtFileList.isChecked():
			...
			track_vals['pattern_include'] = self.ui.lineEdit_patternInTrack.text()
			track_vals['pattern_exclude'] = self.ui.lineEdit_patternExcTrack.text()
		return track_vals
