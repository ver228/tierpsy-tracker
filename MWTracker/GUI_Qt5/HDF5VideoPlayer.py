...
class HDF5VideoPlayer_GUI(QtWidgets.QMainWindow):
    ...
    def readCurrentFrame(self):
        ...
        self.frame_qimg = QtGui.QImage(self.frame_img.data, 
            self.image_width, self.image_height, self.frame_img.strides[0], QtGui.QImage.Format_Indexed8)
        self.frame_qimg = self.frame_qimg.convertToFormat(QtGui.QImage.Format_RGB32, QtCore.Qt.AutoColor)
        
    #file dialog to the the hdf5 file
    def getVideoFile(self):
        vfilename, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Find HDF5 video file", 
    ...
    def updateVideoFile(self, vfilename):
        ...
        try:
            ...
            self.fid = -1
            self.image_group = -1
            QtWidgets.QMessageBox.critical(self, '', "The selected file is not a valid .hdf5. Please select a valid file",
    ...
    def updateGroupNames(self):
        ...
        if not valid_groups:
            QtWidgets.QMessageBox.critical(self, '', "No valid video groups were found. Dataset with three dimensions and uint8 data type. Closing file.",
                    QtWidgets.QMessageBox.Ok)
            self.fid.close()
            self.image_group = -1
            self.mainImage.cleanCanvas()
    ...
    def updateImGroup(self):
        ...
        if not self.h5path in self.fid:
            self.mainImage.cleanCanvas()
            self.image_group == -1
            QtWidgets.QMessageBox.critical(self, 'The groupset path does not exist', "The groupset path does not exists. You must specify a valid groupset path",
                    QtWidgets.QMessageBox.Ok)
            return
        ...
        if len(self.image_group.shape) != 3:
            self.mainImage.cleanCanvas()
            self.image_group == -1
            QtWidgets.QMessageBox.critical(self, 'Invalid groupset', "Invalid groupset. The groupset must have three dimensions",
                    QtWidgets.QMessageBox.Ok)
