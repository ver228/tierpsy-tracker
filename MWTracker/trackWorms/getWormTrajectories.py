...
def _getWormFeatures(worm_cnt, ROI_image, ROI_bbox, current_frame, thresh, min_area):
...
def _joinConsecutiveFrames(index_list_prev, coord, coord_prev, area, area_prev, tot_worms, max_allowed_dist, area_ratio_lim):
...
area_ratio_lim = (0.5, 2), buffer_size = 25, worm_bw_thresh_factor = 1., strel_size = (5,5)):
    '''
    ...
    tables.open_file(trajectories_file, mode = 'w') as feature_fid:
        ...
        for frame_number in range(initial_frame, last_frame, buffer_size):
            ...
            #change from bool to uint since same datatype is required in opencv
            ...
            for ROI_cnt in ROI_cnts:
                ...
                #select ROI for all buffer slides and apply a median filter to sharp edgers
                ...
                #calculate threshold using the nonzero pixels.  Using the buffer instead of a single image, improves the threshold calculation, since better statistics are recoverd
                ...
                for buff_ind in range(image_buffer.shape[0]):
                    ...
                    ROI_worms, hierarchy = _getWormContours(ROI_buffer_med[buff_ind,:,:], thresh, strel_size)
                    ...
                    for worm_ind, worm_cnt in enumerate(ROI_worms):
                        ...
                        mask_features = _getWormFeatures(worm_cnt, ROI_buffer[buff_ind,:,:], ROI_bbox, current_frame, thresh, min_area)
                    ...
                    if len(frame_features)>0:
                        ...
                        index_list, tot_worms = _joinConsecutiveFrames(index_list_prev, coord, coord_prev, area, area_prev, 
                            tot_worms, max_allowed_dist, area_ratio_lim)
                        
                        #append the new feature list to the pytable
                        dum_column = len(index_list)*(-1,) #used to leave space for worm_index_joined
                        frame_features = list(zip(*([index_list, dum_column] + frame_features)))
