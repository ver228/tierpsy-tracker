...
def startTask(cmd_fun, curr_args, current_tasks, num_tasks):
    cmd = cmd_fun(*curr_args)
    print(cmd)
    current_tasks.append(sp.Popen(cmd, shell='True'))
    num_tasks += 1
    print('%s : started.' % curr_args[0])
    return current_tasks, num_tasks

def runMultiSubproc(cmd_fun, args_list, max_num_process = 6):
    tot_tasks = len(args_list)
    ...
    num_tasks = 0; 
    for curr_args in args_list[0:max_num_process]:
        current_tasks, num_tasks = startTask(cmd_fun, curr_args, current_tasks, num_tasks)
