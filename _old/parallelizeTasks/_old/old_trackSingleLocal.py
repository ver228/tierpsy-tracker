...
if __name__ == '__main__':
    try:
        ...
        start_point = getStartingPoint(
            trajectories_file, skeletons_file, features_file)
        if start_point == checkpoint['END']:
            print(
                'Results files were completed. Remove %s, %s %s if you want to recalculated them again.' %
                (trajectories_file, skeletons_file, features_file))
        else:
            ...
            if not os.path.exists(tmp_mask_file):
                ...
                if tmp_mask_file != masked_image_file:
                    print(
                        "Copying masked file %s into the temporary directory %s" %
                        (masked_image_file, tmp_masked_dir))
            ...
            if start_point > checkpoint[
                    'TRAJ_CREATE'] and not os.path.exists(trajectories_tmp):
                shutil.copy(trajectories_file, tmp_results_dir)

            if start_point > checkpoint[
                    'SKE_CREATE'] and not os.path.exists(features_tmp):
                shutil.copy(skeletons_file, tmp_results_dir)

            if start_point > checkpoint[
                    'FEAT_CREATE'] and not os.path.exists(features_tmp):
                shutil.copy(features_file, tmp_results_dir)

            getTrajectoriesWorkerL(
                tmp_mask_file,
                tmp_results_dir,
                param_file=json_file,
                overwrite=False)

            print(
                "Copying result files into the final directory %s" %
                results_dir)
