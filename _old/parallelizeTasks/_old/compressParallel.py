...
def videoProcessingWorker(
    ...
    masked_image_file = masked_image_file.replace(
        ' ', '_')  # spaces are evil get ride of them
...
#    video_ext = 'mpjg'
    ...
    for opt, arg in opts:
        ...
        if opt == '-h':
            print(
                'compress_worm_videos.py -i <inputfile> -o <outputfile> -e <videoextension>')
            sys.exit()
        elif opt in ("-i", "--ifile"):
            video_dir = arg
        elif opt in ("-o", "--ofile"):
    ...
    # start the parallizeTask object, obtain the queue where the progress
    # status is stored
