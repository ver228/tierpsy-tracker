...
expected_size = floor(
    ...
    float(
        full_dataset.attrs['save_interval']) +
    1)

if expected_size < full_dataset.shape[0]:
    # much faster than
    # #full_dataset[0::full_dataset.attrs['save_interval'],:,:]

    N = int(floor(full_dataset.shape[0] /
                  full_dataset.attrs['save_interval'])) + 1
    d1 = np.zeros(
        (N,
         full_dataset.shape[1],
         full_dataset.shape[2]),
        dtype=full_dataset.dtype)
    for ii, kk in enumerate(
            range(0, full_dataset.shape[0], full_dataset.attrs['save_interval'])):
        print ii, kk
