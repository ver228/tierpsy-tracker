...
class ReadVideoffmpeg:
    ...
    def __init__(self, fileName, width=-1, height=-1):
        ...
        command = [ffmpeg_cmd,
                   ...
                   '-threads', '0',
                   '-vcodec', 'rawvideo', '-']
        # use a buffer size as small as possible, makes things faster
        self.pipe = sp.Popen(command, stdout=sp.PIPE, bufsize=self.tot_pix)
...
def writeFullFramesTiff(masked_image_file, tiff_file=-1, reduce_fractor=8):
    ...
    for frame in range(expected_size):
        I_worms[frame, :, :] = cv2.resize(
            mask_fid["/full_data"][frame, :, :], im_size)
...
def getImageROI(image):
    # if it is needed to keep the original image then use
    # "image=getImageROI(np.copy(image))"
...
def compressVideo(
    ...
    # MAX_FRAMES : maximum number of frames to be analyzed. Set this value to
    # a large value to compress all the video
...
if __name__ == '__main__':
    ...
    save_dir = '/Users/ajaver/Downloads/wetransfer-2af646/'
    ...
    for ind in [3]:  # range(1,7):
        ...
        plt.figure()
        plt.imshow(mask_fid['/mask'][0, :, :],
                   interpolation='none', cmap='gray')

        plt.figure()
        plt.imshow(mask_fid['/full_data'][-1, :, :],
                   interpolation='none', cmap='gray')
        mask_fid.close()
