...
def analyze_image(parent_conn, frame, image, bgnd_im, bgnd_mask):
    ...
    parent_conn.send(
        {'frame': frame, 'image': image, 'props_list': props_list})
    parent_conn.close()
...
def get_indexList(
        ...
        totWorms,
        max_allow_dist=10.0):
    # get the indexes of the next worms from their nearest neightbors using
...
def analyze_frames(
    ...
    totWorms, indexList, speed = get_indexList(
        coord, coord_prev, indexListPrev, totWorms)
...
#'''OPEN THE VIDEO FILE AND EXTRACT SOME PARAMETERS'''
    ...
    # initialize video to the initial position of the background buffer
...
#'''MAIN LOOP STARTS'''
    for ind_frame in range(max_frame):
        ...
        if nChunk_prev != nChunk:
            ...
            bgnd_mask = cv2.adaptiveThreshold(
                bgnd_im, 1, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 61, 35)
        nChunk_prev = nChunk
        ...
        if processes.qsize() >= N_processes:
            coord_prev, indexListPrev, totWorms = analyze_frames(
                processes, mask_dataset, results_list, coord_prev, indexListPrev, totWorms)
