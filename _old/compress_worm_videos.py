    ...
    def __init__(self, fileName, width=-1, height=-1):
        ...
        if width <= 0 or height <= 0:
            try:
                ...
                pipe.terminate()
                # the frame size is somewhere printed at the beggining by
                # ffmpeg
        ...
        command = [ffmpeg_cmd,
                   ...
                   '-threads', '0',
                   '-vcodec', 'rawvideo', '-']
        # use devnull to avoid printing the ffmpeg command output in the screen
...
def getROIMask(image, min_area=100, max_area=5000):
    ...
    # adaptative threshold is the best way to find possible worms. I setup the
    # parameters manually, they seems to work fine if there is no condensation
    # in the sample
...
def compressVideo(video_file, masked_image_file, buffer_size=25,
    ...
    if check_empty:
        # select 200 random index use to check if the image is empty
        rand_ind = np.random.randint(0, im_height * im_width - 1, (200))
    ...
    while frame_number < max_frame:
        ...
        # check if the 200 randomly selected index are equal (much faster that
        # check the whole image)
        if check_empty and np.all(
            image.flat[rand_ind] -
            np.mean(
                image.flat[rand_ind]) == 0):
            continue
        ...
        # Resize mask array every 1000 frames (doing this every frame does not
        # impact much the performance)
        ...
        if frame_number % save_full_interval == 1:
            ...
            # just to be sure that the index we are saving in is what we what
            # we are expecting
            assert(
                np.floor(
                    frame_number /
        ...
        if ind_buff == 0:
            Ibuff = np.zeros(
                (buffer_size, im_width, im_height), dtype=np.uint8)
        ...
        if ind_buff == buffer_size - 1:
            ...
            mask_dataset[(frame_number - buffer_size)
                          :frame_number, :, :] = Ibuff
...
def writeDownsampledVideo(
    ...
    # if no save_video_file is given, the save name will be derived from
    # masked_image_file
    if save_video_file == -1:
        save_video_file = os.path.splitext(masked_image_file)[
            0] + '_downsampled.avi'
    ...
    command = ['ffmpeg',
               ...
               '-vcodec', 'rawvideo',
               # size of one frame
               '-s', '%ix%i' % (I_worms.shape[-1], I_worms.shape[-2]),
               '-pix_fmt', 'gray',
...
def writeFullFramesTiff(
    ...
    '''
    # if no tiff_file is given, the save name will be derived from
    # masked_image_file
    if tiff_file == -1:
    ...
    expected_size = int(
        np.floor(
            mask_fid["/mask"].shape[0] /
            float(
                mask_fid["/full_data"].attrs['save_interval']) +
            1))
    ...
    im_size = tuple(
        np.array(mask_fid["/full_data"].shape)[1:] / reduce_fractor)
    reduce_factor = (im_size[-1], im_size[-2])
    I_worms = np.zeros((expected_size, im_size[0], im_size[1]), dtype=np.uint8)

    sendQueueOrPrint(
        status_queue,
        'Reading for data the tiff file...',
        base_name)

    for frame_number in range(expected_size):
        I_worms[frame_number, :, :] = cv2.resize(
            mask_fid["/full_data"][frame_number, :, :], reduce_factor)
...
if __name__ == '__main__':
    ...
    # '/Volumes/Mrc-pc/20150309/'
    video_dir = '/Users/ajaver/Desktop/Gecko_compressed/pruebaa/'
    # '/Volumes/behavgenom$/GeckoVideo/Compressed/20150309/'
    save_dir = '/Users/ajaver/Desktop/Gecko_compressed/pruebaa/'
...
#
    ...
    base_name_list = [os.path.splitext(x)[0]
                      for x in file_list if (video_ext in x)]
    ...
    for nChunk in range(0, len(base_name_list), max_num_process):
        ...
        while any(i.is_alive() for i in workers):
            # I made this value larger because I can only save the output in a
            # file on a schedule task with "at". Refreshing it too frequently
            # will produce a huge file.
            time.sleep(10.0)
