...
class MWTrackerViewer_GUI(QMainWindow):
    ...
    def calcIndFeat(self):
        ...
        # start the analysis
        trajectories_data = self.trajectories_data[
            self.trajectories_data['worm_label'] == 1]
    ...
    def getVideoFile(self):
        vfilename, _ = QFileDialog.getOpenFileName(
    ...
    def updateVideoFile(self):
        if not os.path.exists(self.vfilename):
            QMessageBox.critical(
                self,
                'The hdf5 video file does not exists',
                "The hdf5 video file does not exists. Please select a valid file",
                QMessageBox.Ok)
            return
        ...
        if os.path.exists(dum):
            ...
            self.basename = self.vfilename.rpartition(
                os.sep)[-1].rpartition('.')[0]
    ...
    def updateGroupNames(self):
        ...
        def geth5name(name, dat):
            if isinstance(
                    dat, h5py.Dataset) and len(
                    dat.shape) == 3 and dat.dtype == np.uint8:
        ...
        if not valid_groups:
            QMessageBox.critical(
                self,
                '',
                "No valid video groups were found. Dataset with three dimensions and uint8 data type.",
                QMessageBox.Ok)
            return
    ...
    def getSkelFile(self):
        self.skel_file, _ = QFileDialog.getOpenFileName(
    ...
    def updateSkelFile(self):
        ...
        with pd.HDFStore(self.skel_file, 'r') as ske_file_id:
            ...
            if not 'worm_index_N' in self.trajectories_data.columns:
                ...
                self.trajectories_data['worm_index_N'] = self.trajectories_data[
                    'worm_index_joined']
    ...
    def updateImGroup(self):
        ...
        if self.h5path not in self.fid:
            ...
            QMessageBox.critical(
                self,
                'The groupset path does not exists',
    ...
    def imSldReleased(self):
        ...
        if self.image_group != -1:
            self.frame_number = round(
                (self.tot_frames - 1) * self.ui.imageSlider.value() / 100)
            self.ui.spinBox_frame.setValue(self.frame_number)

    # frame spin box, important since I would update the image by changing the
    # spinBox value
    ...
    def updateImage(self):
        ...
        if isinstance(self.trajectories_data, pd.DataFrame):
            try:
                # self.frame_data =
                # self.trajectories_data[self.trajectories_data['frame_number']
                # ==
                # self.frame_number]#self.trajectories_data.get_group(self.frame_number)
                self.frame_data = self.traj_time_grouped.get_group(
                    self.frame_number)
                self.frame_data = self.frame_data[
                    self.frame_data[self.worm_index_str] >= 0]
            except KeyError:
                self.frame_data = -1
            ...
            if isinstance(self.frame_data, pd.DataFrame) and \
                ...
                for row_id, row_data in self.frame_data.iterrows():
                    ...
                    if not (
                            x == x) or not (
                            y == y):  # check if the coordinates are nan
                        continue
                    ...
                    painter.drawText(
                        x, y, str(int(row_data[self.worm_index_str])))
    ...
    def updateROIcanvas(
        ...
        worm_roi, roi_corner = getWormROI(self.original_image, roi_data[
                                          'coord_x'], roi_data['coord_y'], roi_data['roi_size'])
        ...
        if isDrawSkel:
            if roi_data['has_skeleton'] == 1:
                ...
                if 'is_good_skel' in roi_data and roi_data[
                        'is_good_skel'] == 0:
                    self.skel_colors = {
                        'skeleton': (
                ...
                worm_mask = worm_mask.scaled(
                    canvas_size, canvas_size, Qt.KeepAspectRatio)  # , Qt.SmoothTransformation)
    ...
    def joinTraj(self):
        ...
        if not (worm_ind1 in self.frame_data[
                'worm_index_N'].values or worm_ind2 in self.frame_data['worm_index_N'].values):
