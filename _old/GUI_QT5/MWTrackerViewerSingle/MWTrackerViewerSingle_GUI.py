...
class MWTrackerViewerSingle_GUI(HDF5videoViewer_GUI):
    ...
    def drawSkel(self, worm_img, worm_qimg, row_data, roi_corner=(0, 0)):
        if not self.skel_file or not isinstance(
    ...
    def drawThreshMask(self, worm_img, worm_qimg, row_data, read_center=True):
        ...
        if read_center:
            worm_cnt, _ = binaryMask2Contour(worm_mask, roi_center_x=row_data[
                                             'coord_y'], roi_center_y=row_data['coord_x'], min_mask_area=min_mask_area)
        else:
            worm_cnt, _ = binaryMask2Contour(
                worm_mask, min_mask_area=min_mask_area)
